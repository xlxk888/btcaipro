...
Stock-token discovery follows the same read/write separation. `api/stock-tokens/markets.js` only reads the shared registry. The persistent worker refreshes it every five minutes when deployed; the public repository also includes `.github/workflows/stock-token-refresh.yml` as the production scheduler for deployments whose Vercel plan does not support frequent Cron Jobs. It calls the idempotent bulk refresh endpoint and never fans out from a page request. Set the same optional `CRON_SECRET` in Vercel and `STOCK_TOKEN_CRON_SECRET` in GitHub Actions to require bearer authentication.
