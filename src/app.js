...
export function createApp({ store, cache, worker, config, startedAt = Date.now() }) {
  return async function app(request, response) {
    ...
    if (url.pathname === '/api/events' || url.pathname === '/api/events/latest') {
      ...
      const events = store.listEvents({ limit: latest ? 1 : (Number(url.searchParams.get('limit')) || 50), asset: url.searchParams.get('asset')?.toUpperCase(), type: url.searchParams.get('type') });
      return send(response, 200, latest ? { data: events[0] || null } : { data: events, count: events.length }, cors);
    }
    if (url.pathname === '/api/sources/status') return send(response, 200, { data: store.listSourceStatuses(), cache: cache.status() }, cors);
